...
namespace GameProject
{
    ...
    public class TeddyBear
    {
        ...
        public void Update(GameTime gameTime)
        {
            ...
            // firing projectle after each delay
			elapsedShotTime += gameTime.ElapsedGameTime.Milliseconds;
			if (elapsedShotTime > firingDelay) {
				elapsedShotTime = 0;

				//randomly getting delay interval
				firingDelay = GetRandomFiringDelay ();

				//shooting at the burger
				Projectile projectile = new Projectile(ProjectileType.TeddyBear,Game1.GetProjectileSprite(ProjectileType.TeddyBear),drawRectangle.Center.X,drawRectangle.Center.Y+GameConstants.TEDDY_BEAR_PROJECTILE_OFFSET,GetProjectileYVelocity());
