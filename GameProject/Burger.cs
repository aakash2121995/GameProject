    ...
    public class Burger
    {
        ...
        public void Update(GameTime gameTime, KeyboardState keyboard)
        {
			...
			if (health > 0) {
				...
				/*//Centering the burger at cursor
				drawRectangle.X = mouse.X - (int)drawRectangle.Width/2;
				drawRectangle.Y = mouse.Y - (int)drawRectangle.Width/2;
				*/
				...
				//clamping the burger into the window
             ...
             // Creating a French fry if button is pressed and shooting is allowed
			if (keyboard.IsKeyDown(Keys.Space) &&canShoot) {
				Projectile projectile = new Projectile (ProjectileType.FrenchFries, Game1.GetProjectileSprite (ProjectileType.FrenchFries), drawRectangle.Center.X,
				                                       drawRectangle.Center.Y - GameConstants.FRENCH_FRIES_PROJECTILE_OFFSET, GameConstants.FRENCH_FRIES_PROJECTILE_SPEED);
				Game1.AddProjectile (projectile);
				shootSound.Play ();
				canShoot = false;
			}

			//allowing shooting if button is released
